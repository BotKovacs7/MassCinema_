...
def open_main_window():
    ...
    # Automatikusan nyissa meg a jegyfoglalás ablakot
    root.after(100, jegyfoglalas.open)
    ...
    for idx, film in enumerate(films_col1):
        try:
            img = ImageTk.PhotoImage(Image.open(f"./images/movie{idx+1}.jpg"))
            ttk.Button(col1, image=img, width=30, command=lambda f=film: open_film_window(f)).pack(pady=10)
            # Megőrizzük a képreferenciát, hogy ne dobja ki a GC
            col1.image = img
    ...
    for idx, film in enumerate(films_col2):
        try:
            img = ImageTk.PhotoImage(Image.open(f"./images/movie{idx+mid+1}.jpg"))
            ttk.Button(col2, image=img, width=30, command=lambda f=film: open_film_window(f)).pack(pady=10)
            col2.image = img
        except Exception as e:
            print(f"Hiba a kép betöltésénél: {e}")
